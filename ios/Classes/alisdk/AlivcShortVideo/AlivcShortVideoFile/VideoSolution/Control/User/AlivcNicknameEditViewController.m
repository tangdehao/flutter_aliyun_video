...
- (void)viewDidLoad {
    ...
    //右上角的完成
    UIBarButtonItem *doneItem = [[UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"完成", nil) style:UIBarButtonItemStylePlain target:self action:@selector(done)];
    ...
    if (self.isQuVideo) {
        self.nameTF.text = [AliVideoClientUser shared].nickName;
    }else{
...
- (void)done{
    ...
    if ([self getToInt:self.nameTF.text] > 12){
        [MBProgressHUD showMessage:[@"nick_no_more_than_12" localString] inView:self.view];
        return;
    }
    
    if ([self.nameTF.text isEqualToString:@""] || !self.nameTF.text) {
        [MBProgressHUD showMessage:[@"输入不可为空" localString] inView:self.view];
        return;
    }

    __weak typeof(self)weakSelf = self;
    if (_isQuVideo) {
        [AlivcQuUserManager updateNickname:self.nameTF.text withToken:[AliVideoClientUser shared].token userId:[AliVideoClientUser shared].userID success:^{
            ...
            self.updateNickNameCompltion(nil);
        } failure:^(NSString * _Nonnull errDes) {
            self.updateNickNameCompltion(errDes);
        }];
